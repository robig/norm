...
public class StandardSqlMaker implements SqlMaker {
    ...
    public StandardSqlMaker(Database db) {
        this.db = db;
    }
    ...
    private void makeRelations(StandardPojoInfo pi) {
        for (Property prop : pi.propertyMap.values()) {
            if (!prop.isRelation || prop.relationClass == null) {
                continue;
            }

            final StandardPojoInfo dest = getPojoInfo(prop.relationClass);

            AttributeConverter c = (AttributeConverter) Proxy.newProxyInstance(getClass().getClassLoader(),
                    new Class[] { AttributeConverter.class },
                    new InvocationHandler() {
                        ...
                        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                            ...
                            if (method.getName().equals("convertToDatabaseColumn") && args != null
                                    && args.length == 1) {
                                Object o = args[0];
                                return dest.getValue(o, dest.primaryKeyName);
                            }
                            // public X convertToEntityAttribute (Y dbData)
                            if (method.getName().equals("convertToEntityAttribute") && args != null
                                    && args.length == 1) {
                                Object y = args[0];
                                if (y instanceof List<?>)
                                    return y; // no conversion
                                return db.where(dest.primaryKeyName + "=?", y)
                                        .first(prop.relationClass);
                            }
                            return null;
                        }
    ...
    public void makeUpdateSql(StandardPojoInfo pojoInfo) {
        ...
        for (Property prop : pojoInfo.propertyMap.values()) {
            ...
            if (prop.isGenerated || !prop.hasColumn) {
                continue;
            }
